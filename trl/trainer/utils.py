...
class DataCollatorForCompletionOnlyLM(DataCollatorForLanguageModeling):
    ...
    def torch_call(self, examples: list[Union[list[int], Any, dict[str, Any]]]) -> dict[
        str, Any]:
        ...
        if self.instruction_template is None:
            for i in range(len(examples)):
                ...
                for idx in np.where(batch["labels"][i] == self.response_token_ids[0])[0]:
                    ...
                    if (
                            ...
                            == batch["labels"][i][
                               idx: idx + len(self.response_token_ids)].tolist()
                    ):
            ...
            for i in range(len(examples)):
                ...
                for assistant_idx in \
                np.where(batch["labels"][i] == self.response_token_ids[0])[0]:
                    # find the indexes of the start of a response.
                    if (
                            self.response_token_ids
                            == batch["labels"][i][assistant_idx: assistant_idx + len(
                        self.response_token_ids)].tolist()
                    ):
                ...
                for human_idx in np.where(batch["labels"][i] == human_token_ids[0])[0]:
                    ...
                    if human_token_ids == batch["labels"][i][human_idx: human_idx + len(
                            human_token_ids)].tolist():
        ...
        if self.padding_free:
            ...
            indices_q = torch.arange(
                flattened_position_ids.size(0), device=flattened_position_ids.device,
                dtype=torch.int32
            )
            batch["cu_seq_lens_q"] = torch.cat(
                (
                    ...
                    torch.tensor(
                        flattened_position_ids.size(), device=flattened_position_ids.device,
                        dtype=torch.int32
                    ),
                )
            ).unsqueeze(0)
...
class DataCollatorForChatML:
    ...
    def __call__(self, examples: list[dict[str, Any]]) -> dict[str, torch.Tensor]:
        ...
        input_ids = pad(input_ids, padding_side="left",
        ...
        prompt_attention_mask = [torch.tensor(mask, dtype=torch.long) for mask in
                                 prompt_attention_mask]
        prompts_input_ids = pad(prompts_input_ids, padding_side="left",
                                padding_value=self.tokenizer.pad_token_id)
        prompt_attention_mask = pad(prompt_attention_mask, padding_side="left",
                                    padding_value=0)
...
def pad(tensors: list[torch.Tensor], padding_value: int = 0,
    ...
    output = torch.full((len(tensors), *output_shape), padding_value, dtype=tensors[0].dtype,
        ...
        for k in features[0].keys():
            if k.endswith(("_input_ids", "_attention_mask", "_labels", "_pixel_values")):
                if self.is_encoder_decoder:
                    ...
                    padded_batch[k] = pad(to_pad, padding_value=padding_value,
...
class ConstantLengthDataset(IterableDataset):
    ...
    def __init__(
        ...
        column_names = (
            dataset.column_names if isinstance(dataset, (
            datasets.Dataset, datasets.IterableDataset)) else None
        )
...
class RunningMoments:
    ...
    def save_to_json(self, json_path: str):
        ...
        if self.accelerator.is_main_process:
            save_dict = dataclasses.asdict(self,
                                           dict_factory=lambda x: {k: v for (k, v) in x if
                                                                   k != "accelerator"})
...
def get_global_statistics(
    ...
    sum_and_count = torch.tensor([xs.sum(), (xs.numel() if mask is None else mask.sum())],
                                 device=xs.device)
...
def compute_accuracy(eval_pred: EvalPrediction) -> dict[str, float]:
    ...
    if predictions.ndim == 3:
        ...
        predictions = np.array(
            [p for prediction, label in zip(predictions, labels) for (p, lbl) in
             zip(prediction, label) if lbl != -100]
        )
...
def pad_to_length(tensor: torch.Tensor, length: int, pad_value: Union[int, float],
...
class PerPromptStatTracker:
    ...
    def get_stats(self):
        return {k: {"mean": np.mean(v), "std": np.std(v), "count": len(v)} for k, v in
                self.stats.items()}
...
def get_exp_cap(value, decimal=4):
    ...
    return torch.floor(
        vdtype_log_max * 10 ** decimal) / 10 ** decimal if decimal > 0 else vdtype_log_max
...
def prepare_deepspeed(
        model: torch.nn.Module, per_device_train_batch_size: int, fp16: bool = False,
        bf16: bool = False
):
    """
...
def generate(
        lm_backbone: torch.nn.Module, queries: torch.Tensor, pad_token_id: int,
...
def batch_generation(
    ...
    # padding tensors
    padded_query_responses = pad(query_responses, padding_value=pad_token_id,
                                 padding_side="right")
    padded_logitss = pad(logitss, padding_value=0, padding_side="right")

    # reshaping
    padded_query_responses = padded_query_responses.view(-1,
                                                         padded_query_responses.shape[-1])[
                             :batch_size]
    ...
    if bos_token_id is not None:
        if prompt_len_input_ids == 0 or bos_token_id != prompt_tokens["prompt_input_ids"][0]:
            ...
            prompt_tokens["prompt_attention_mask"] = [1] + prompt_tokens[
                "prompt_attention_mask"]
        if chosen_prompt_len_input_ids == 0 or bos_token_id != \
                chosen_tokens["prompt_input_ids"][0]:
            chosen_tokens["prompt_input_ids"] = [bos_token_id] + chosen_tokens[
                "prompt_input_ids"]
            chosen_tokens["prompt_attention_mask"] = [1] + chosen_tokens[
                "prompt_attention_mask"]
        if rejected_prompt_len_input_ids == 0 or bos_token_id != \
                rejected_tokens["prompt_input_ids"][0]:
...
def add_eos_token_if_needed(
        eos_token_id: int, chosen_tokens: dict[str, list[int]],
        rejected_tokens: dict[str, list[int]]
):
    if len(chosen_tokens["input_ids"]) == 0 or eos_token_id != chosen_tokens["input_ids"][-1]:
        chosen_tokens["input_ids"].append(eos_token_id)
        chosen_tokens["attention_mask"].append(1)
    if len(rejected_tokens["input_ids"]) == 0 or eos_token_id != rejected_tokens["input_ids"][
        -1]:
...
def decode_and_strip_padding(inputs: torch.Tensor, tokenizer: PreTrainedTokenizerBase) -> \
list[str]:
    """
...
def flush_left(mask: torch.Tensor, *tensors: torch.Tensor) -> tuple[torch.Tensor, ...]:
    ...
    first_empty_col = torch.nonzero(empty_cols)[0].item() if empty_cols.any() else mask.size(
        1)
...
def selective_log_softmax(logits, index):
    ...
    if logits.dtype in [torch.float32, torch.float64]:
        ...
        per_token_logps = []
        for row_logits, row_labels in zip(logits,
                                          index):  # loop to reduce peak mem consumption
...
def print_prompt_completions_sample(prompts: list[str], completions: list[str],
                                    rewards: list[int], step: int) -> None:
    """
    ...
    if not is_rich_available():
        raise ImportError(
            "This feature requires `rich` to be installed. Please install it first: `pip install rich`")
