...
class RLOOConfig(OnPolicyConfig):
    ...
    token_level_kl: bool = field(
        default=False,
        metadata={
            "help": "Whether to use token-level KL penalty or sequence-level KL penalty"},
    )
    ...
    stepwise: bool = field(
        default=False,
        metadata={
            "help": "This setting applied for estimating the KL divergence between the current and reference policy. "
                    "The default is the conventional MC estimator that has high variance, and the stepwise option "
                    "gives unbiased estimate while having variance not greater than the MC estimator."
        },
    )
